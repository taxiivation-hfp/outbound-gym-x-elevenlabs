/**
 * The intelligence page's view: health of the gym, then why members leave.
 *
 * PLACEHOLDER UI for the pass-one additions — `MembershipAndMoney`, `Engagement`
 * and `ReasonThemes` are plain on purpose and pass three replaces them
 * wholesale. The "why they leave" sections below them were moved here unchanged
 * from app/intelligence/page.tsx, so the page can be rendered from data alone
 * (the guards render it with no calls, and with too few reasons to summarise).
 */
import { Engagement, MembershipAndMoney } from "@/components/intelligence/GymHealth";
    ...
    <main className="mx-auto w-full max-w-[1400px] px-6 py-8 sm:px-8 sm:py-10">
      <header>
        ...
        <p className="mt-3 text-xs text-zinc-600">
          Member data as of {data.as_of} · {data.calls.dials} dials · {data.calls.completed} completed ·{" "}
      ...
      <MembershipAndMoney health={data.health} />
      <Engagement health={data.health} />

      <h2 className="mt-12 text-2xl font-black uppercase tracking-tight">Why they leave</h2>
      ...
      {hasReasons && (
        <>
          ...
          <ReasonThemes themes={data.why_they_leave.themes} />
